...
import { User, Table, MenuItem, Client, Order, Schedule, Role } from '../types';
import {
  TrendingUp,
  ShoppingBag,
  Users,
...
export default function AdminDashboard({
  ...
  const handleAddTable = (e: React.FormEvent) => {
    ...
    const newTable: Table = {
      id: `T-${tableLabel.trim()}`,
      label: `T-${tableLabel.trim()}`,
  ...
  const handleDeleteTable = (id: string) => {
    if (confirm('¿Realmente eliminar esta mesa?')) {
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'mesas' && (
              ...
              >
                ...
                {/* Mesas grid viewer */}
                <div className="bg-slate-900/30 border border-slate-800 rounded-2xl p-5 space-y-4">
                  <h3 className="text-sm font-bold text-white border-b border-slate-800 pb-3">Croquis y Capacidad Física del Salón</h3>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {tables.map((t) => (
                      <div key={t.id} className="bg-slate-900 border border-slate-850 p-4 rounded-2xl relative group flex flex-col justify-between min-h-[110px]">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="text-[10px] font-mono font-bold text-slate-500 block">Identificación</span>
                            <span className="text-white font-extrabold text-sm font-mono">{t.label}</span>
                          </div>
                          
                          <button
                            id={`del_table_${t.id}`}
                            onClick={() => handleDeleteTable(t.id)}
                            className="text-slate-600 hover:text-red-400 p-1 rounded hover:bg-slate-850 transition-colors opacity-0 group-hover:opacity-100"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>

                        <div className="flex justify-between items-end border-t border-slate-850 pt-2.5 mt-2.5">
                          <span className="text-xs text-slate-400">{t.capacity} Personas (Pax)</span>
                          <span className={`w-2.5 h-2.5 rounded-full ${t.status === 'Libre' ? 'bg-emerald-500' : t.status === 'Ocupada' ? 'bg-red-500' : t.status === 'Reservada' ? 'bg-amber-400' : 'bg-slate-500'}`} />
                        </div>
                      </div>
                    ))}
                  </div>
